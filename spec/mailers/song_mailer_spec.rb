...
RSpec.describe SongMailer do
  ...
  describe '#pickup' do
    ...
    let(:song) { create(:song, name: 'くちなしの丘', artist: '原田知世', live:, members: users.map(&:member) + [email_rejected_user.member, member]) }
    ...
    it 'renders the headers and the body' do
      ...
      expect(mail.body.to_s).to eq read_fixture('pickup.txt')
    end
  end
end
