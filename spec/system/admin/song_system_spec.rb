...
RSpec.describe 'Admin song system:', type: :system do
  specify 'An admin creates a song', js: true do
    ...
    # Then
    expect(page).to have_selector '.playing-form', count: 1
    ...
    # Then
    expect(page).to have_selector '.playing-form', count: 3
    ...
    # Then
    expect(page).to have_selector '.playing-form', count: 3
    expect(page).to have_selector '.playing-form-visible-fields', count: 2

    # When
    fill_in '曲名', with: 'Lion'
    fill_in 'アーティスト名', with: 'DEPAPEPE'
    within all('.playing-form-visible-fields')[0] do
      fill_in '楽器', with: 'Gt'
      select member1.joined_year_and_name, from: 'メンバー'
    end
    within all('.playing-form-visible-fields')[1] do
      fill_in '楽器', with: 'Gt'
    ...
    # Then
    expect(page).to have_content 'Lion / DEPAPEPE を追加しました'
  ...
  specify 'An admin edits a song', js: true do
    ...
    expect(song.playings.count).to eq 2
  end
end
